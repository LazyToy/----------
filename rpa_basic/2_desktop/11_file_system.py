...
# 파일 경로에서 폴더 정보 가져오기
# print(os.path.dirname(r"C:\Users\bea99\OneDrive\바탕 화면\나도코딩 업무자동화\my_file.txt"))
...
# # 파일의 마지막 접근 날짜
# atime = os.path.getmtime("run_btn.png")
# print(datetime.datetime.fromtimestamp(mtime).strftime("%Y%m%d %H:%m:%S"))
...
# *.xlsx, *.txt, 자동화*.png
# import fnmatch
# patten = "*.png " # .py 로 끝나는 모든 파일
# result = []
# for root, dirs, files in os.walk("."):
#     for name in files:
#         if fnmatch.fnmatch(name, patten): # 이름이 패터과 일치하면
#             result.append(os.path.join(root, name))
# print(result)
...
# 만약에 지정된 경로에 해당한느 파일/ 폴더가 없다면?
# print(os.path.isfile("askdjh.png")) # 해당하는 폴더, 파일이 없다면 False가 출력됨
...
# shutil.rmtree("new_folder") # 폴더 안이 비어 있지 않아도 완전 삭제 가능
...
shutil.move("test_folder2", "test_folder3") # 해당 폴더명이 없으면 변경되는 효과
